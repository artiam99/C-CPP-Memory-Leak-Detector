...
void *xcalloc(object_db_t *object_db, char *struct_name, int units)
{
    ...
    add_object_to_object_db(object_db, ptr, units, struct_rec, MLD_FALSE);  /* xmalloc by default set the object as non-root */
...
{    
    ...
    printf("|---------------------------------------------------------------------------------------------------------------|\n");
    printf("| %-3d ptr = %-10p | next = %-15p | units = %-4d | struct_name = %-10s | is_root = %s |\n", 
...
/* Level 2 Pseudocode : This function explore the direct childs of obj_rec and mark them visited. Note that obj_rec must have already visted. */
    ...
    for(i = 0 ; i < parent_obj_rec->units ; i++)
    {
        ...
        for(n_fields = 0; n_fields < parent_struct_rec->n_fields; n_fields++)
        {
            ...
            switch(field_info->dtype)
            {
                ...
                /*child_object_address now stores the address of the next object in the graph. It could be NULL, Handle that as well*/
                if(!child_object_address) continue;

                child_object_rec = object_db_look_up(object_db, child_object_address);

                assert(child_object_rec);
                /* Since we are able to reach this child object "child_object_rec" from parent object "parent_obj_ptr", mark this
...
/* Level 1 Pseudocode : We will traverse the graph starting from root objects and mark all reachable nodes as visited */
void run_mld_algorithm(object_db_t *object_db)
{
    ...
    /* Step 2 : Get the first root object from the object db, it could be present anywhere in object db.
       If there are multiple roots in object db return the first one, we can start mld algorithm from any root object */
    ...
    while(root_obj)
    {
        ...
        {
            /* It means, all objects reachable from this root_obj has already been explored, no need to do it again, else you will end up in infinite loop.
